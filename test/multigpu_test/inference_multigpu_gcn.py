...
def load_dataset(args):
    ...
    return m, nnz, col, row, edge_val, in_feat, weight_s, num_classes, row_ptr, col_idx, col_ptr, row_idx
...
def main(args):
    m, nnz, col, row, edge_val, in_feat, weight_s, num_classes, row_ptr, col_idx, col_ptr, row_idx = load_dataset(args)
    ...
    if args.partition_alg == "sa":
        ...
        p = torch.searchsorted(row_ptr, torch.tensor([0, int(nnz / 4), int(nnz / 2), int(nnz * 3 / 4), nnz], dtype=torch.int64))
        q = torch.searchsorted(col_ptr, torch.tensor([0, int(nnz / 4), int(nnz / 2), int(nnz * 3 / 4), nnz], dtype=torch.int64))
        ...
        row_ptr_s, col_idx_s, edge_val_s, _, _ = partion_graph(args, row, col, edge_val, m, initial_state[0], initial_state[1])
        ...
        while current_temp > final_temp:
            ...
            perturb[0][1:4] += (torch.randn(3) * 10000).to(torch.int64)
            perturb[1][1:4] += (torch.randn(3) * 10000).to(torch.int64)
            
            # Constraining inputs
            for i in range(1,4):
                if (perturb[0][i] < perturb[0][i-1]): perturb[0][i] = perturb[0][i-1]
                if (perturb[1][i] < perturb[1][i-1]): perturb[1][i] = perturb[1][i-1]
            ...
            if cost_diff < 0 or random.random() < math.exp(- cost_diff * 10 / current_temp):
            ...
            print("Temp {} deg: {} (best: {})".format(current_temp, current_cost, solution_cost), flush=True)
        ...
        row_ptr_s, col_idx_s, edge_val_s, p, q = partion_graph(args, row, col, edge_val, m, p, q)
        out_feat_our, inference_time = multigpu_gcnconv.multigpu_gcn_inference(
            nnz,
            args.num_layers,
            args.num_hidden,
